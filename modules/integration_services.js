...
try{
  ...
  integrationClient.invokeOperation(operation_name,headers,params, 
			function(result) {
			  		callback(result);
			}, 
			function(error) {
    ...
    if(service_name==="LinkedInPostURL" && error.httpStatusCode===400)
      alert("Duplicate post not allowed");
    else
			alert("Integration Service Failure :" +service_name+"  "+JSON.stringify(error));
			});
