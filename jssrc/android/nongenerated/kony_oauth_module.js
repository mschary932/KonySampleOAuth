...
function integSuccessCallback(res) {
    if (res["httpStatusCode"] === 200 && res["profile"] != undefined) displayKonyProfile(res["profile"]);
...
function kOAuth() {
    ...
    if (userId !== null || userId !== undefined) {
        userId = userId.trim();
        if (userId == "") alert("please enter valid email id");
    } else {
    ...
    try {
        ...
        intgService.invokeOperation(MBaaSConfig.KonyOAuthServiceOperation, {}, {
            "userid": userId,
            "password": password
